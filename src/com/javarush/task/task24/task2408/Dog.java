    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable{
            private String getName(){
                return String.format("%s%s%s", this.getSuperQuotes(), name, this.getSuperQuotes());
            }

            @Override
            public String say() {
                if(i < 1) return String.format("%s спит.", this.getName());
                else {
                    StringBuilder sb = new StringBuilder();
                    for (int a = 0; a < i; a++){
                        sb.append('а');
                    }
                    return String.format("%s лает г%sв! %s", this.getName(), sb, formatter.format(new Date().getTime()));
                }
            }
        }
        return new DogPet();
