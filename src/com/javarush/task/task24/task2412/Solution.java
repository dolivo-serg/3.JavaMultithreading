...
//                секунд (сверху самые новые), потом по прибыли от положительных к отрицательным
                ...
                result = profit(stock2).toString().compareTo(profit(stock1).toString());
                return result;
            }
        });
    }
    private static Object profit(Stock stock){
        if(stock.containsKey("change"))
            return ((double)stock.get("change")) * 1000;
        else
            return ((double)stock.get("last")) - ((double)stock.get("open"))*1000 ;
    }
